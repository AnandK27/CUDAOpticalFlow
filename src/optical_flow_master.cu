...
__global__ void cudaComputeOpticalFlow3(const float* Ix, const float* Iy, const float* It, int width, int height, int stride, float* u, float* v)
{
    ...
    if (y < height && x < width)
    {
        Ix2[threadIdx.y + WIN_SIZE / 2][threadIdx.x + WIN_SIZE / 2] = Ix[idx] * Ix[idx];
        Iy2[threadIdx.y + WIN_SIZE / 2][threadIdx.x + WIN_SIZE / 2] = Iy[idx] * Iy[idx];
        IxIy[threadIdx.y + WIN_SIZE / 2][threadIdx.x + WIN_SIZE / 2] = Ix[idx] * Iy[idx];
        IxIt[threadIdx.y + WIN_SIZE / 2][threadIdx.x + WIN_SIZE / 2] = Ix[idx] * It[idx];
        IyIt[threadIdx.y + WIN_SIZE / 2][threadIdx.x + WIN_SIZE / 2] = Iy[idx] * It[idx];
    }

    // Left Edge
    if (threadIdx.x < WIN_SIZE / 2 && x >= WIN_SIZE / 2)
    {
        int temp_idx = idx - WIN_SIZE / 2;
        Ix2[threadIdx.y + WIN_SIZE / 2][threadIdx.x] = Ix[temp_idx] * Ix[temp_idx];
        Iy2[threadIdx.y + WIN_SIZE / 2][threadIdx.x] = Iy[temp_idx] * Iy[temp_idx];
        IxIy[threadIdx.y + WIN_SIZE / 2][threadIdx.x] = Ix[temp_idx] * Iy[temp_idx];
        IxIt[threadIdx.y + WIN_SIZE / 2][threadIdx.x] = Ix[temp_idx] * It[temp_idx];
        IyIt[threadIdx.y + WIN_SIZE / 2][threadIdx.x] = Iy[temp_idx] * It[temp_idx];
    ...
    {
        int temp_idx = idx + WIN_SIZE / 2;
        Ix2[threadIdx.y + WIN_SIZE / 2][threadIdx.x + WIN_SIZE - 1] = Ix[temp_idx] * Ix[temp_idx];
        Iy2[threadIdx.y + WIN_SIZE / 2][threadIdx.x + WIN_SIZE - 1] = Iy[temp_idx] * Iy[temp_idx];
        IxIy[threadIdx.y + WIN_SIZE / 2][threadIdx.x + WIN_SIZE - 1] = Ix[temp_idx] * Iy[temp_idx];
        IxIt[threadIdx.y + WIN_SIZE / 2][threadIdx.x + WIN_SIZE - 1] = Ix[temp_idx] * It[temp_idx];
        IyIt[threadIdx.y + WIN_SIZE / 2][threadIdx.x + WIN_SIZE - 1] = Iy[temp_idx] * It[temp_idx];
    }

    // Top Edge
    if (threadIdx.y < WIN_SIZE / 2 && y >= WIN_SIZE / 2)
    {
        int temp_idx = idx - WIN_SIZE / 2 * stride;
        Ix2[threadIdx.y][threadIdx.x + WIN_SIZE / 2] = Ix[temp_idx] * Ix[temp_idx];
        Iy2[threadIdx.y][threadIdx.x + WIN_SIZE / 2] = Iy[temp_idx] * Iy[temp_idx];
        IxIy[threadIdx.y][threadIdx.x + WIN_SIZE / 2] = Ix[temp_idx] * Iy[temp_idx];
        IxIt[threadIdx.y][threadIdx.x + WIN_SIZE / 2] = Ix[temp_idx] * It[temp_idx];
        IyIt[threadIdx.y][threadIdx.x + WIN_SIZE / 2] = Iy[temp_idx] * It[temp_idx];
    }
    ...
    {
        int temp_idx = idx + WIN_SIZE / 2 * stride;
        Ix2[threadIdx.y + WIN_SIZE - 1][threadIdx.x + WIN_SIZE / 2] = Ix[temp_idx] * Ix[temp_idx];
        Iy2[threadIdx.y + WIN_SIZE - 1][threadIdx.x + WIN_SIZE / 2] = Iy[temp_idx] * Iy[temp_idx];
        IxIy[threadIdx.y + WIN_SIZE - 1][threadIdx.x + WIN_SIZE / 2] = Ix[temp_idx] * Iy[temp_idx];
        IxIt[threadIdx.y + WIN_SIZE - 1][threadIdx.x + WIN_SIZE / 2] = Ix[temp_idx] * It[temp_idx];
        IyIt[threadIdx.y + WIN_SIZE - 1][threadIdx.x + WIN_SIZE / 2] = Iy[temp_idx] * It[temp_idx];
    }
    ...
    if (threadIdx.x == 0 && threadIdx.y == 0 && x > 0 && y > 0)
    {
        for (int wy = -WIN_SIZE / 2; wy < 0; wy++)
        {
            for (int wx = -WIN_SIZE / 2; wx < 0; wx++)
            {
                int temp_idx = idx + (wy * stride) + wx;
                Ix2[threadIdx.y + WIN_SIZE / 2 + wy][threadIdx.x + WIN_SIZE / 2 + wx] = Ix[temp_idx] * Ix[temp_idx];
                Iy2[threadIdx.y + WIN_SIZE / 2 + wy][threadIdx.x + WIN_SIZE / 2 + wx] = Iy[temp_idx] * Iy[temp_idx];
                IxIy[threadIdx.y + WIN_SIZE / 2 + wy][threadIdx.x + WIN_SIZE / 2 + wx] = Ix[temp_idx] * Iy[temp_idx];
                IxIt[threadIdx.y + WIN_SIZE / 2 + wy][threadIdx.x + WIN_SIZE / 2 + wx] = Ix[temp_idx] * It[temp_idx];
                IyIt[threadIdx.y + WIN_SIZE / 2 + wy][threadIdx.x + WIN_SIZE / 2 + wx] = Iy[temp_idx] * It[temp_idx];
            }
        }
    }
    ...
    for (int wy = -WIN_SIZE / 2; wy <= WIN_SIZE / 2; wy++)
    {
        for (int wx = -WIN_SIZE / 2; wx <= WIN_SIZE / 2; wx++)
        {
            sumIx2 += Ix2[threadIdx.y + wy + WIN_SIZE / 2][threadIdx.x + wx + WIN_SIZE / 2];
            sumIy2 += Iy2[threadIdx.y + wy + WIN_SIZE / 2][threadIdx.x + wx + WIN_SIZE / 2];
            sumIxIy += IxIy[threadIdx.y + wy + WIN_SIZE / 2][threadIdx.x + wx + WIN_SIZE / 2];
            sumIxIt += IxIt[threadIdx.y + wy + WIN_SIZE / 2][threadIdx.x + wx + WIN_SIZE / 2];
            sumIyIt += IyIt[threadIdx.y + wy + WIN_SIZE / 2][threadIdx.x + wx + WIN_SIZE / 2];
        }
    }
...
int main(int argc, char** argv) {
    ...
    printf("Device Name: %s, reservedSharedMemPerBlock : %d, sharedMemPerBlock: %d, MaxThreadsPerMultiProcessor: %d\n", 
        deviceProp.name, deviceProp.reservedSharedMemPerBlock, deviceProp.sharedMemPerBlock, deviceProp.maxThreadsPerMultiProcessor);
