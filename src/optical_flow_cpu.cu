...
__global__ void cudaComputeOpticalFlow3(const float* Ix, const float* Iy, const float* It, int width, int height, int stride, float* u, float* v)
{
    ...
    int x = threadIdx.x + blockDim.x * blockIdx.x + (WIN_SIZE / 2);
    int y = threadIdx.y + blockDim.y * blockIdx.y + (WIN_SIZE / 2);
    ...
    if (threadIdx.x >= blockDim.x - WIN_SIZE / 2)
    {
    ...
    if (threadIdx.y > -blockDim.y - WIN_SIZE / 2)
    {
    ...
    for (int wy = -WIN_SIZE / 2; wy <= WIN_SIZE / 2; wy++) {
        for (int wx = -WIN_SIZE / 2; wx <= WIN_SIZE / 2; wx++) {
            int idx = (threadIdx.y + wy + WIN_SIZE / 2) * stride + (threadIdx.x + wx + WIN_SIZE / 2);
            sumIx2 += Ix_shared[idx] * Ix_shared[idx];
            sumIy2 += Iy_shared[idx] * Iy_shared[idx];
            sumIxIy += Ix_shared[idx] * Iy_shared[idx];
            sumIxIt += Ix_shared[idx] * It_shared[idx];
            sumIyIt += Iy_shared[idx] * It_shared[idx];
        }
    }
...
int main(int argc, char **argv) {
    ...
    while (frame_num < 3)
    {
        ...
        cudaComputeOpticalFlow2 << <numBlocks, threadsPerBlock >> > (d_Ix, d_Iy, d_It, width, height, stride, d_u, d_v);
        ...
        float calc_fps = frame_num / (float)difftime(tock, tick);
